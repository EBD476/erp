...
@section('content')
    ...
    <div class="content persian">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <a href="{{route('product-middle-part.create')}}" class="btn btn-primary float-left mb-lg-2">
                        <i class="tim-icons icon-simple-add"></i>
                        {{__('New Product Middle Part')}}
                    </a>
                                ...
                                <div class="card-body">
                                    <div class="table-responsive table-hover">
                                        <table id="table" class="table" cellspacing="0" width="100%">
                                            ...
                                            <tbody>
                                            @foreach($product_part as $product_middle_parts)
                                                <tr>
                                                    <td>
                                                        {{$product_middle_parts->id}}
                                                    </td>
                                                    @foreach($product as $products)
                                                        @if($products->id == $product_middle_parts->hpp_product_id)
                                                        <td>
                                                    ...
                                                    @foreach($middle_part as $middle_parts)
                                                        @if($middle_parts->id == $product_middle_parts->hpp_middle_part_id)
                                                            <td>
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-right"
                                                                ...
                                                                <a class="dropdown-item"
                                                                   href="{{route('product-middle-part.edit',$product_middle_parts->id)}}"
                                                                >{{__('Edit')}}</a>
                                                                <form id="-form-delete{{$product_middle_parts->id}}"
                                                                      style="display: none;" method="POST"
                                                                      action="{{route('product-middle-part.destroy',$product_middle_parts->id)}}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                </form>
                                                                <a class="dropdown-item"
                                                                   onclick="if(confirm('آیا از حذف این قطعه اطمینان دارید؟')){
                                                                           event.preventDefault();
                                                                           document.getElementById('-form-delete{{$product_middle_parts->id}}').submit();
                                                                           }else {
